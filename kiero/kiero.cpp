...
HWND SafeCreateWindow(_In_ DWORD dwExStyle,
...
{
	typedef HWND(__stdcall* tCreateWindowExA)(
	...
	static tCreateWindowExA at = NULL;

	if (!at)
	{
		auto niggerware = (uintptr_t)CreateWindowExA;
		auto relativeoffset = *(int*)(niggerware + 0x1);
		auto pJump = (niggerware + relativeoffset + 0x5);
		byte check = *(byte*)(pJump);
		if (check == 0xFF)
			at = (tCreateWindowExA) * (uintptr_t*)(pJump - 0xE + 0x6);
		else
			at = (tCreateWindowExA)(niggerware);
	}

	return at(dwExStyle, lpClassName, lpWindowName, dwStyle, X, Y, nWidth, nHeight, hWndParent, hMenu, hInstance, lpParam);
}
...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.lpszClassName = KIERO_TEXT("SL-AC");
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			// REMEMBER TO UNHOOK CREATEWINDOWEXA (IMPORTANT) 
			HWND window = ::CreateWindowExA(0l, windowClass.lpszClassName, KIERO_TEXT("SL-AC"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);
